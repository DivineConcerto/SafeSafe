...
def exif_size(img):
    """Returns corrected PIL images size (width, height) considering EXIF orientation."""
...
def create_dataloader(
    ...
    """Creates and returns a configured DataLoader instance for loading and processing images datasets."""
    if rect and shuffle:
...
class LoadScreenshots:
    ...
    def __init__(self, source, img_size=640, stride=32, auto=True, transforms=None):
        """
        Initializes a screenshot dataloader for YOLOv5 with specified source region, images size, stride, auto, and
        transforms.
...
class LoadImages:
    ...
    def _cv2_rotate(self, im):
        """Rotates a cv2 images based on its orientation; supports 0, 90, and 180 degrees rotations."""
...
def img2label_paths(img_paths):
    """Generates label file paths from corresponding images file paths by replacing `/images/` with `/labels/` and
    extension with `.txt`.
    """
...
class LoadImagesAndLabels(Dataset):
    ...
    def load_image(self, i):
        """
        Loads an images by index, returning the images, its original dimensions, and resized dimensions.
    ...
    def cache_images_to_disk(self, i):
        """Saves an images to disk as an *.npy file for quicker loading, identified by index `i`."""
        f = self.npy_files[i]
        if not f.exists():
            np.save(f.as_posix(), cv2.imread(self.im_files[i]))

    def load_mosaic(self, index):
        """Loads a 4-images mosaic for YOLOv5, combining 1 selected and 3 random images, with labels and segments."""
        labels4, segments4 = [], []
        s = self.img_size
    ...
    def load_mosaic9(self, index):
        """Loads 1 images + 8 random images into a 9-images mosaic for augmented YOLOv5 training, returning labels and
        segments.
        """
        labels9, segments9 = [], []
        s = self.img_size
...
def verify_image_label(args):
    """Verifies a single images-label pair, ensuring images format, size, and legal label values."""
...
class HUBDatasetStats:
    ...
    def _hub_ops(self, f, max_dim=1920):
        """Resizes and saves an images at reduced quality for web/app viewing, supporting both PIL and OpenCV."""
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, augment, imgsz, cache=False):
        """Initializes YOLOv5 Classification Dataset with optional caching, augmentations, and transforms for images
        classification.
        """
    ...
    def __getitem__(self, i):
        """Fetches and transforms an images sample by index, supporting RAM/disk caching and Augmentations."""
...
def create_classification_dataloader(
    ...
    """Creates a DataLoader for images classification, supporting caching, augmentation, and distributed training."""
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
